...
__date__ = "01-10-2019"
__copyright__ = "Copyright 2019, JKU Institute for Machine Learning"
...
class LossFunction(Module):
    ...
    def __init__(self, reduction='mean', target_grads=False):
        ...
        def _reshape_grad(grad, shape):
            grad = np.atleast_2d(grad)
            return np.broadcast_to(grad, shape)

        if reduction == 'mean':
            self.reduce = lambda x: np.mean(np.atleast_2d(x), axis=0)
            self.reduce_grad = lambda d, s: _reshape_grad(d, s) / s[0]
        elif reduction == 'sum':
            self.reduce = lambda x: np.sum(np.atleast_2d(x), axis=0)
            self.reduce_grad = _reshape_grad
        elif reduction == 'none':
    ...
    def backward(self, *grads):
        # peek shape cache
        shape, _ = self._shape_cache[-1]
        grads = (self.reduce_grad(grad, shape) for grad in grads)
        return super().backward(*grads)
